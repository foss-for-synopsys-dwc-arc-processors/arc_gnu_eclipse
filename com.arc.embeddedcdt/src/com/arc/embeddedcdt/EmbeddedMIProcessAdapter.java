...
/**
 * This is a 1:1 copy of the original MIProcessAdapter, except that gdb is now
 * started(!) in the root of the active project.  Useful for gdb incarnations
 * which are doing things right before they are executing the "cd" command line
 * parameter
 * 
 * @author hardy
 */
public class EmbeddedMIProcessAdapter implements MIProcess
{
	Process fGDBProcess;
	...
	public EmbeddedMIProcessAdapter(String[] args, IProgressMonitor monitor) throws IOException {
		this(args, 0, monitor);
	}

	public EmbeddedMIProcessAdapter(String[] args, int launchTimeout, IProgressMonitor monitor) throws IOException {
		this(args, launchTimeout, monitor, null);
	}

	public EmbeddedMIProcessAdapter(String[] args, int launchTimeout, IProgressMonitor monitor, Launch launch) throws IOException {
		this.launch = launch;
	...
	protected Process getGDBProcess(String[] args, int launchTimeout, IProgressMonitor monitor) throws IOException {
		final Process pgdb = ProcessFactory.getFactory().exec(args, (launch != null) ? launch.getEnvironment() : null,
				                               (launch != null) ? launch.getStartDir() : null);
