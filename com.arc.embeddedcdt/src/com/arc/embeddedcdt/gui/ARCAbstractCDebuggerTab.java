...
import org.eclipse.swt.widgets.Label;
    public abstract class ARCAbstractCDebuggerTab extends CLaunchConfigurationTab {
    ...
    */
    ...
    public String arc_remote_gdb="";
    protected void loadDebuggerCombo(ICDebugConfiguration[] debugConfigs, String current) {
    fDCombo.removeAll();

    for (int i = 0; i < debugConfigs.length; i++) {
    	if(debugConfigs[i].getName().indexOf("ARC remote")>-1){
        fDCombo.add(debugConfigs[i].getName());
        fDCombo.setData(arc_remote_gdb, debugConfigs[i]);
    	}
    }
    fPageUpdated = false;
    //if (select != -1) {
    ...
    */
    ...
    return (ICDebugConfiguration)fDCombo.getData(arc_remote_gdb);
    }
    }
